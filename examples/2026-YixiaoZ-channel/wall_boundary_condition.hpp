...
namespace WallBoundaryCondition {
  ...
  template<class Real>
  auto build_solver(const Real dx, const Real radius,
      const Real vapor_density, const Real cv, const Real kappa) {
    ...
    const Real boundary_k = vapor_density * kappa * cv / dx;
